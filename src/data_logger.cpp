...
inline fs::path make_save_path(const std::string& folder)
{
    std::error_code err;
...
inline bool save_camera_parameters(const CameraParameters& params, const std::string& file_name)
{
    YAML::Emitter out;
    out << YAML::BeginMap;
...
inline CameraParameters make_cinfo_parameters(const std::string& topic, ros::NodeHandle& nh, const ros::Duration& timeout = ros::Duration(0))
{
...
fs::path BaseLogger::get_output_path() const
{
    return output_path_;
}

void BaseLogger::run() const
{
    ros::Rate rate(10);
    while (ros::ok()){
        ros::spinOnce();
    ...
    BaseLogger()
{
    ...
    subcriber_ = private_nh_.subscribe(image_topic_name_, MESSAGE_QUEUE, &ImageLogger::image_cb_, this);
}
